...
function makePost() {
	...
	httpRequest.onreadystatechange = function() { postHandler(httpRequest, document.getElementById('author').value, document.getElementById('text').value) };

	httpRequest.open("POST", "/new_message");
	httpRequest.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');

	
	var data = "author=" + document.getElementById('author').value + "&chatname=" + document.getElementById('chatname').value + "&text=" + document.getElementById('text').value;
	console.log(data);
...
function postHandler(httpRequest, author, text) {
	if (httpRequest.readyState === XMLHttpRequest.DONE) {
		if (httpRequest.status === 200) {
			...
			temp.innerHTML = '<li id="user"><p><span class="author">' + author + ':</span> ' + text;
...
function makePoll() {
	...
	httpRequest.onreadystatechange = function() { pollHandler(httpRequest) };
			...
			for (var i = 0; i < msgs.length; i++) {
				...
				if (msgs[i].text !== undefined) {
					...
					if(author != msgs[i].author){
						li.innerHTML = '<li id="other"><p><span class="author">'+ msgs[i].author + ':</span>' + msgs[i].text;
					}
					ul.appendChild(li);
				}
			}
